...
def test_include_only_usdt_pairs_true_does_not_depend_on_quote_allowlist() -> None:
    cfg = {
        "universe_filters": {
            "include_only_usdt_pairs": True,
            "quote_allowlist": ["USDC"],
            "market_cap": {"min_usd": 1, "max_usd": 10_000_000_000},
            "volume": {"min_quote_volume_24h": 0},
        },
        "filters": {"exclusion_patterns": []},
    ...
    f = UniverseFilters(cfg)
    out = f.apply_all([
        {"symbol": "AAAUSDT", "base": "AAA", "quote_volume_24h": 1, "market_cap": 100},
        {"symbol": "AAAUSDC", "base": "AAA", "quote_volume_24h": 1, "market_cap": 100},
    ])

    assert [x["symbol"] for x in out] == ["AAAUSDT"]


def test_legacy_exclusion_patterns_null_falls_back_to_new_exclusions() -> None:
    cfg = {
        "filters": {"exclusion_patterns": None},
        "exclusions": {
            "exclude_stablecoins": True,
            "stablecoin_patterns": ["USD"],
            "exclude_wrapped_tokens": False,
    ...
    f = UniverseFilters(cfg)
    out = f.apply_all([
        {"symbol": "AAAUSDT", "base": "AAAUSD", "quote": "USDT", "quote_volume_24h": 1, "market_cap": 100},
        {"symbol": "BBBUSDT", "base": "BBB", "quote": "USDT", "quote_volume_24h": 1, "market_cap": 100},
    ])

    assert [x["symbol"] for x in out] == ["BBBUSDT"]
