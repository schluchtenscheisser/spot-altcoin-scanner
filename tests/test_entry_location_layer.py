...
def _record(
    *,
    ...
    return {
        ...
        "universe": {"candidate_excluded": False},
...
def test_candidate_excluded_uses_top_level_path_not_universe_block() -> None:
    top_level_excluded = _record(excluded=True, tradeable=True)
    top_level_excluded["universe"]["candidate_excluded"] = False
    assert evaluate_entry_location(top_level_excluded, _cfg()).entry_action_hint == "monitor_only"

    nested_only_excluded = _record(excluded=False, tradeable=True)
    nested_only_excluded["universe"]["candidate_excluded"] = True
    assert evaluate_entry_location(nested_only_excluded, _cfg()).entry_action_hint == "buy_now_candidate"
