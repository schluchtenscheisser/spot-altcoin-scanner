...
**Last Updated:** 2026-06-14 11:35 UTC  
...
- Codex must not edit legacy/reference-only modules merely because they appear in this map.
...
### 📄 `scanner/axes/__init__.py`

**Functions:** —

**Imports:** `scanner.axes.models, scanner.axes.tier1, scanner.axes.tier2`

---

### 📄 `scanner/axes/models.py`

**Classes:** `Tier1AxisBundle, Tier2AxisBundle`
...
**Functions:** `_is_finite_number, norm_linear_clamped, norm_linear_clamped_inv, norm_piecewise_linear, weighted_mean`

**Module Variables:** `ratio, sf, total_weight, xf, xs, ys`
...
**Functions:** `_aggregate, _extract_input, _is_ok, _lin_inv_score, _lin_score, _pw_score, _resolve_axes_cfg, compute_tier1_axes`

**Module Variables:** `anchor_map, axes, axes_cfg, comp_cfg, comp_fields, comp_scores, dist, dist_score, exp_cfg, exp_fields` _(+30 more)_

**Imports:** `__future__, scanner.axes.models, scanner.axes.normalization, scanner.features.models, typing`

---

### 📄 `scanner/axes/tier2.py`

**Functions:** `_aggregate, _axis_score, _extract_input, _is_ok, _lin_inv_score, _lin_score, _pw_score, _valid_impulse, compute_tier2_axes`

**Module Variables:** `axes_cfg, base_cfg, base_specs, block, high, min_ratio, path, pb_cfg, pb_specs, points` _(+13 more)_
...
**Module Variables:** `BASE_URL, cache_key, data, elapsed, exchange_info, logger, params, response, results, retry_after` _(+4 more)_
...
### 📄 `scanner/decision/__init__.py`

**Functions:** —

**Imports:** `scanner.decision.buckets, scanner.decision.models, scanner.decision.ranking, scanner.decision.reasons`

---

### 📄 `scanner/decision/buckets.py`

**Functions:** `_build, _coerce_market_phase_confidence_for_rankability, _is_finite, _make_discarded, assign_bucket`
...
**Module Variables:** `ENTRY_ACTION_HINT_VALUES, ENTRY_LOCATION_STATUS_VALUES, acceptable, auxiliary, bucket, decision, decision_block, default, dist, entry_location` _(+24 more)_
...
**Classes:** `ConfirmedReason, DecisionBucket, DecisionBundle, DiscardedReason, EarlyReason, ExecutionInputContract, LateMonitorReason, RankedDecision, ReasonAssignment, WatchlistReason`

**Functions:** `__post_init__`

**Module Variables:** `CHASED_STATE, CONFIRMED_CANDIDATES, CONFIRMED_EXECUTION_OK, CONFIRMED_EXECUTION_PENDING, CONFIRMED_PATTERN_UNRESOLVED, CONFIRMED_PATTERN_VALID, CONFIRMED_STATE_VALID, DISCARDED, EARLY_CANDIDATES, EARLY_EXECUTION_OK` _(+19 more)_

**Imports:** `__future__, dataclasses, enum, math, scanner.entry.models`

---

### 📄 `scanner/decision/ranking.py`

**Functions:** `_coerce_score_input_for_non_gated_path, _require_finite, _sort_nullable_desc, compute_priority_score, map_execution_grade, rank_coins`

**Module Variables:** `bucket, bucket_order, grade, mapping, raw, sorted_records`

**Imports:** `__future__, math, scanner.decision.models`

---

### 📄 `scanner/decision/reasons.py`

**Functions:** `assign_reasons`

**Imports:** `__future__, scanner.decision.models`
...
### 📄 `scanner/evaluation/__init__.py`

**Functions:** —

**Imports:** `dataset_export`

---

### 📄 `scanner/evaluation/dataset_export.py`

**Functions:** `run_evaluation_export`

**Module Variables:** `eval_cfg, exports_dir, k, manifest_path, payload, raw_cfg, replay_diag_path, replay_dir, signal_path, summary_path` _(+3 more)_

**Imports:** `__future__, hashlib, json, pathlib, scanner.config, scanner.evaluation.forward_returns, scanner.evaluation.replay, typing`
...
**Functions:** `__init__, _symbol_dir, closed_bars_as_of, load_symbol_timeframe`
...
**Classes:** `HistoricalProductionAdapter, ReplayProductionAdapterProtocol, ReplayProductionOutput`

**Functions:** `__call__, _bars_from_df, _build_bar_clock_context, _finite_float, _state_ctx`

**Module Variables:** `bar_clock_context, bootstrap, bundle, close_time_utc_ms, current_daily_bar, cycle_ctx, d1, daily_close_time_utc_ms, entry, h4` _(+19 more)_

**Imports:** `__future__, dataclasses, datetime, logging, math, pandas, scanner.axes.tier1, scanner.axes.tier2` _(+9 more)_

---

### 📄 `scanner/evaluation/historical_replay/replay_runner.py`

**Functions:** `_atomic_write_json, _bar_day, _compute_manifest_symbol_counts, _generate_replay_id, _map_bucket, _validate_resume_state_store, get_current_daily_bar, has_current_day_4h_coverage, run_replay`

**Module Variables:** `adapter, adapter_out, adapter_used, admitted_count, as_of, bar_id, chunk_dir, chunk_id, chunk_manifest, chunks_completed` _(+42 more)_
...
### 📄 `scanner/execution/__init__.py`

**Functions:** —

**Imports:** `adapter, grading`
...
**Classes:** `ExecutionGradeResult, _LegacyExecutionCfg`

**Functions:** `__init__, _has_invalid_levels, _map_reason, grade_execution_orderbook`
...
### 📄 `scanner/features/__init__.py`

**Functions:** —

**Imports:** `scanner.features.bundle, scanner.features.models, scanner.features.raw_1d, scanner.features.raw_4h, scanner.features.shared`
...
**Classes:** `FeatureBundle, RawFeatures1D, RawFeatures4H, RawFeaturesShared`

**Functions:** —

**Module Variables:** `FeatureStatus`
...
**Functions:** `_atr_wilder, _bars_to_series, _default_raw_1d, _ema_sma_bootstrap, _has_gap_in_last_window, _rolling_rank, _validate_context, _validate_symbol, compute_raw_1d, pct, ratio, st, streak_above`

**Module Variables:** `a, alpha, atr, atr_pct, atr_pct_hist, atr_pct_rank, atr_series, bars_above20, bars_above50, bars_since_low` _(+72 more)_

**Imports:** `__future__, math, scanner.features.models, statistics, typing`

---

### 📄 `scanner/features/raw_4h.py`

**Functions:** `_bars_to_series, _close_vs_high20_4h_pct, _default_raw_4h, _validate_symbol, apct, compute_raw_4h, pct, ratio, st, streak_above`

**Module Variables:** `anchor, anchor_status, atr, atr_hist, atr_last, atr_pct, atr_rank, bars20, bars50, bars_high20` _(+87 more)_

**Imports:** `__future__, math, scanner.features.models, scanner.features.raw_1d, statistics, typing`

---

### 📄 `scanner/features/shared.py`

**Functions:** `_validate_bar_clock_context, _validate_symbol, compute_raw_shared`

**Module Variables:** `atr_1d, normalized, range_width`

**Imports:** `__future__, math, scanner.features.models, typing`
...
**Imports:** `diagnostics, report_builder, schema`

---

### 📄 `scanner/output/diagnostics.py`

**Functions:** `_atomic_write_bytes, serialize_diagnostics_jsonl_lines, write_symbol_diagnostics_jsonl_gz`

**Module Variables:** `gz_bytes, payload, validated`

**Imports:** `__future__, gzip, json, os, pathlib, schema, tempfile, typing`

---

### 📄 `scanner/output/diagnostics_serialization.py`

**Functions:** `_dataclass_attr_map, _json_value, _raw_4h_ok_value, serialize_axes_block, serialize_cycle_block, serialize_decision_block, serialize_entry_location_inputs, serialize_invalidation_block, serialize_pattern_block, serialize_phase_block, serialize_reasons_block, serialize_state_block`

**Module Variables:** `bars_since_cycle_end, cycle_end_bar_index, cycle_end_timestamp, fields, key, logger, out, patch, previous_setup_cycle_id, raw_4h` _(+5 more)_

**Imports:** `__future__, enum, logging, math, scanner.axes.models, scanner.decision.models, scanner.entry.models, scanner.features.models` _(+3 more)_
...
**Functions:** `__init__, _atomic_write_json, _atomic_write_text, _date_parts_from_daily_bar_id, _sort_newest_first, _update_index_after_run, make_report_builder, write_daily_report, write_run_report`

**Module Variables:** `counts, daily_bar_id, daily_report_path, diagnostics_path_rel, diagnostics_record_count, diagnostics_records_list, excluded_symbols, existing, existing_payload, is_intraday_noop` _(+14 more)_

**Imports:** `__future__, datetime, diagnostics, json, os, pathlib, scanner.config, schema` _(+2 more)_
...
### 📄 `scanner/state/__init__.py`

**Functions:** —

**Imports:** `scanner.state.invalidation, scanner.state.machine, scanner.state.models`

---

### 📄 `scanner/state/cycle.py`

**Classes:** `CycleResolution`
...
**Functions:** `_distance_pct, _interp, _score, compute_state_freshness`

**Module Variables:** `bars_score, distance, distance_score, ratio, settings`

**Imports:** `__future__, math, scanner.config, scanner.state.models`

---

### 📄 `scanner/state/invalidation.py`

**Functions:** `_compute_structural, _compute_timing, _g1_phase_to_none, _g2_insufficient_tier1, _require_input_types, _require_matching_identity, compute_invalidation_and_cycle`

**Module Variables:** `cycle, fields, not_eval, null_like, priority, timing, timing_reason, v1, value`

**Imports:** `__future__, scanner.axes.models, scanner.config, scanner.phase.models, scanner.state.cycle, scanner.state.models`
...
**Classes:** `InvalidationCycleBundle, PersistedStateCycleContext, PersistedStateMachineContext, StateEvaluationDisposition, StateFreshnessBundle, StateMachineBundle, StatePersistencePatch, StateRuntimeContext`

**Functions:** `__post_init__, _is_finite_0_100, _validate_cycle_id, _validate_finite, _validate_non_negative_int, _validate_positive_price`
...
### 📄 `scanner/storage/__init__.py`

**Functions:** —

**Imports:** `repositories, schema, snapshots, sqlite`
...
**Functions:** `apply_state_persistence_patch, get_ohlcv_cache_meta, insert_symbol_run_decision, load_persisted_state_machine_context, ohlcv_bar_exists, read_recent_ohlcv_bars, upsert_ohlcv_cache_meta, upsert_symbol_metadata, write_ohlcv_bars_conflict_strict`

**Module Variables:** `bars, columns, existing, flag, inserted, noop_identical, placeholders, row, rows, same` _(+1 more)_

**Imports:** `__future__, dataclasses, scanner.state.models, sqlite3, typing`
...
**Classes:** `MonthMutabilityPolicy`

**Functions:** `_parse_daily_bar_id, _validate_partition_year_month, _validate_relative_root, _validate_run_id, _validate_symbol, _validate_timeframe, build_ohlcv_history_partition_dir, build_run_manifest_path, build_run_snapshot_dir, is_month_open, month_mutability_policy`

**Module Variables:** `allowed, base_root, normalized, open_month, path, run_date, safe_run_id, sym, tf`

**Imports:** `__future__, dataclasses, datetime, pathlib`

---

### 📄 `scanner/storage/sqlite.py`

**Functions:** `connect_sqlite, init_db`

**Module Variables:** `connection, path`

**Imports:** `__future__, pathlib, schema, sqlite3`
...
**Module Variables:** `DATASET_SCHEMA_VERSION, INTENTIONAL_LEGACY_DEPENDENCIES, LEGACY_EXPORT_BOUNDARY, args, backtest_cfg, candle, config, config_root, current, e2` _(+38 more)_
...
**Classes:** `UniverseClassification`

**Functions:** `_base_symbol, classify_symbol`

**Module Variables:** `ACTIONABLE_CANDIDATE_EXCLUDED_CATEGORIES, CANDIDATE_EXCLUDED_CATEGORIES, base, excluded`
...
| `norm_linear_clamped` | `_is_finite_number` | `ValueError` |
| `norm_linear_clamped_inv` | `_is_finite_number` | `ValueError` |
| `norm_piecewise_linear` | `_is_finite_number` | `RuntimeError`, `ValueError` |
| `weighted_mean` | `_is_finite_number` | `ValueError`, `append`, `isfinite` |

### 📄 scanner/axes/tier1.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_aggregate` | — | `weighted_mean` |
| `_extract_input` | `_is_ok` | — |
| `_lin_inv_score` | — | `norm_linear_clamped_inv` |
| `_lin_score` | — | `norm_linear_clamped` |
| `_pw_score` | — | `norm_piecewise_linear` |
| `_resolve_axes_cfg` | — | `get` |
| `compute_tier1_axes` | `_aggregate`, `_extract_input`, `_lin_inv_score`, `_lin_score`, `_pw_score`, `_resolve_axes_cfg` | `Tier1AxisBundle`, `append`, `get`, `norm_linear_clamped`, `norm_piecewise_linear`, `scorer` |
...
|------------------|----------------|----------------|
| `_aggregate` | — | `weighted_mean` |
| `_axis_score` | `_aggregate`, `_extract_input`, `_lin_inv_score`, `_lin_score`, `_pw_score` | `append` |
| `_extract_input` | `_is_ok` | — |
| `_lin_inv_score` | — | `norm_linear_clamped_inv` |
| `_lin_score` | — | `norm_linear_clamped` |
| `_pw_score` | — | `norm_piecewise_linear` |
| `_valid_impulse` | `_extract_input` | — |
| `compute_tier2_axes` | `_axis_score`, `_valid_impulse` | `Tier2AxisBundle` |
...
| `_parse_points` | `_parse_0_100`, `_raise_invalid` | `append`, `isfinite` |
...
| `exclude_wrapped` | — | `get` |
| `execution` | `resolve_independence_execution_config` | — |
| `feature_layer_config` | `resolve_feature_layer_config` | — |
...
| `priority` | `resolve_priority_config` | — |
...
| `resolve_bucket_config` | `_deep_merge_dicts` | `ValueError`, `get`, `isfinite` |
...
| `resolve_independence_release_retention_config` | `_deep_merge_dicts`, `_raise_invalid` | `get` |
| `resolve_independence_release_snapshots_config` | `_deep_merge_dicts`, `_raise_invalid` | `get`, `strip` |
| `resolve_independence_universe_config` | `_deep_merge_dicts`, `_raise_invalid`, `_read_nested` | `get`, `isfinite` |
| `resolve_invalidation_config` | `_deep_merge_dicts`, `_parse_0_100`, `_raise_invalid` | `get`, `items` |
| `resolve_phase_config` | `_deep_merge_dicts`, `_parse_0_100`, `_parse_ratio`, `_raise_invalid` | `get`, `items` |
| `resolve_priority_config` | `_deep_merge_dicts` | `ValueError`, `get`, `isfinite` |
| `resolve_risk_max_stop_distance_pct` | `_parse` | `ValueError`, `get` |
| `resolve_risk_min_rr_to_target_1` | — | `ValueError`, `get`, `isfinite` |
| `resolve_runner_config` | `_deep_merge_dicts` | `ValueError`, `get` |
| `resolve_state_config` | `_deep_merge_dicts`, `_parse_0_100`, `_parse_points`, `_raise_invalid` | `get` |
...
| `run_mode` | — | `get` |
| `runner` | `resolve_runner_config` | — |
| `scoring_volume_source` | — | `get` |
| `shadow_mode` | — | `get` |
| `shortlist_size` | — | `get` |
| `spec_version` | — | `get` |
| `state` | `resolve_state_config` | — |
| `timezone` | — | `get` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_build` | `_coerce_market_phase_confidence_for_rankability` | `DecisionBundle`, `_coerce_score_input_for_non_gated_path`, `assign_reasons`, `compute_priority_score` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `__post_init__` | — | `ValueError`, `isfinite` |

### 📄 scanner/decision/ranking.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_coerce_score_input_for_non_gated_path` | — | `isfinite` |
| `_require_finite` | — | `TypeError`, `isfinite` |
| `_sort_nullable_desc` | — | `isfinite` |
| `compute_priority_score` | `_require_finite`, `map_execution_grade` | `isfinite` |
| `map_execution_grade` | — | `ValueError` |
| `rank_coins` | `_sort_nullable_desc` | `RankedDecision`, `append`, `get` |

### 📄 scanner/decision/reasons.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `assign_reasons` | — | `ReasonAssignment`, `ValueError` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `run_evaluation_export` | — | `as_posix`, `build_signal_metrics`, `dumps`, `encode`, `get`, `hexdigest`, `mkdir`, `reconstruct_event_timeline`, `resolve_independence_evaluation_config`, `sha256`, `to_parquet`, `write`, `write_text` |
...
| `load_symbol_timeframe` | `_symbol_dir` | `DataFrame`, `ValueError`, `copy`, `exists`, `read_parquet`, `reset_index`, `sort_values`, `to_datetime` |

### 📄 scanner/evaluation/historical_replay/production_adapter.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `__call__` | `_bars_from_df`, `_build_bar_clock_context`, `_state_ctx` | `PersistedStateCycleContext`, `ReplayProductionOutput`, `StateRuntimeContext`, `append`, `asdict`, `build_feature_bundle`, `compute_invalidation_and_cycle`, `compute_phase_interpretation`, `compute_state_machine`, `compute_tier1_axes`, `compute_tier2_axes`, `exception`, `get`, `load_config`, `resolve_entry_pattern`, `to_dict` |
| `_bars_from_df` | `_finite_float` | `SimpleNamespace`, `Timestamp`, `ValueError`, `append`, `get`, `sort_values`, `timestamp`, `to_dict` |
| `_build_bar_clock_context` | — | `Timestamp`, `timestamp`, `tz_convert`, `tz_localize` |
| `_finite_float` | — | `ValueError`, `isfinite` |
| `_state_ctx` | — | `PersistedStateMachineContext`, `get` |
...
| `_generate_replay_id` | — | `now`, `strftime` |
...
| `run_replay` | `_atomic_write_json`, `_compute_manifest_symbol_counts`, `_generate_replay_id`, `_map_bucket`, `_validate_resume_state_store`, `get_current_daily_bar`, `has_current_day_4h_coverage` | `DataFrame`, `HistoricalBarLoader`, `HistoricalProductionAdapter`, `Path`, `ReplayStateStore`, `ValueError`, `adapter`, `append`, `as_posix`, `closed_bars_as_of`, `combine`, `critical`, `dumps`, `exception`, `exists`, `fromisoformat`, `get`, `info`, `is_dir`, `isoformat`, `items`, `iterdir`, `loads`, `mkdir`, `now`, `read_bytes`, `read_text`, `replace`, `scenario_config_hash`, `startswith`, `strftime`, `time`, `timedelta`, `to_parquet`, `unlink`, `update`, `upsert`, `warning`, `write`, `write_bytes` |
...
|------------------|----------------|----------------|
| `_atr_wilder` | — | `append` |
| `_bars_to_series` | — | `TypeError`, `ValueError`, `append`, `isfinite` |
| `_default_raw_1d` | — | `RawFeatures1D`, `append`, `endswith`, `keys` |
| `_rolling_rank` | — | `isfinite` |
| `_validate_context` | — | `TypeError` |
| `_validate_symbol` | — | `TypeError`, `ValueError`, `strip`, `upper` |
| `compute_raw_1d` | `_atr_wilder`, `_bars_to_series`, `_default_raw_1d`, `_ema_sma_bootstrap`, `_has_gap_in_last_window`, `_rolling_rank`, `_validate_context`, `_validate_symbol`, `pct`, `ratio`, `st`, `streak_above` | `RawFeatures1D`, `append`, `get`, `isfinite`, `median`, `sqrt` |
| `pct` | — | `isfinite` |
| `ratio` | — | `isfinite` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_bars_to_series` | — | `TypeError`, `ValueError`, `append` |
| `_close_vs_high20_4h_pct` | — | `isfinite` |
| `_default_raw_4h` | — | `RawFeatures4H`, `endswith`, `keys` |
| `_validate_symbol` | — | `TypeError`, `ValueError`, `strip`, `upper` |
| `apct` | `pct` | — |
| `compute_raw_4h` | `_bars_to_series`, `_close_vs_high20_4h_pct`, `_validate_symbol`, `apct`, `pct`, `ratio`, `st`, `streak_above` | `RawFeatures4H`, `TypeError`, `_atr_wilder`, `_ema_sma_bootstrap`, `_rolling_rank`, `append`, `get`, `isfinite`, `median`, `sqrt` |
| `pct` | — | `isfinite` |
| `ratio` | — | `isfinite` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_validate_bar_clock_context` | — | `TypeError` |
| `_validate_symbol` | — | `TypeError`, `ValueError`, `strip`, `upper` |
| `compute_raw_shared` | `_validate_bar_clock_context`, `_validate_symbol` | `RawFeaturesShared`, `TypeError`, `isfinite` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_atomic_write_bytes` | — | `fdopen`, `fileno`, `flush`, `fsync`, `mkdir`, `mkstemp`, `replace`, `unlink`, `write` |
...
| `_raw_4h_ok_value` | — | `isfinite`, `warning` |
...
| `serialize_entry_location_inputs` | `_raw_4h_ok_value` | — |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `__init__` | — | `resolve_independence_release_reports_config` |
| `_atomic_write_json` | `_atomic_write_text` | `dumps` |
| `_atomic_write_text` | — | `fdopen`, `fileno`, `flush`, `fsync`, `mkdir`, `mkstemp`, `replace`, `unlink`, `write` |
| `_date_parts_from_daily_bar_id` | — | `split`, `validate_daily_bar_id` |
| `_update_index_after_run` | `_atomic_write_json`, `_atomic_write_text`, `_sort_newest_first` | `exists`, `get`, `loads`, `mkdir`, `read_text` |
| `make_report_builder` | — | `ReportBuilder` |
| `write_daily_report` | `_atomic_write_json`, `_date_parts_from_daily_bar_id` | `mkdir` |
| `write_run_report` | `_atomic_write_json`, `_date_parts_from_daily_bar_id`, `_update_index_after_run` | `RunReport`, `as_posix`, `get`, `items`, `keys`, `normalize_counts_by_bucket`, `normalize_symbol_lists`, `relative_to`, `to_dict`, `validate_diagnostics_record`, `write_symbol_diagnostics_jsonl_gz` |

### 📄 scanner/output/schema.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_require_bool` | — | `ValueError` |
| `_require_nullable_bool` | — | `ValueError` |
...
| `validate_diagnostics_record` | `_require_bool`, `_require_nullable_bool`, `_require_nullable_str`, `_require_symbol`, `_validate_diagnostics_invariants`, `is_operational_trade_candidate`, `normalize_entry_location_block`, `normalize_entry_location_inputs`, `validate_as_of_utc`, `validate_daily_bar_id`, `validate_intraday_bar_id`, `validate_run_id`, `validate_scan_mode` | `ValueError`, `get`, `isfinite` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `resolve_cycle_state` | `_phase_floor_recovered` | `CycleResolution` |

### 📄 scanner/state/freshness.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_score` | `_distance_pct`, `_interp` | — |
| `compute_state_freshness` | `_score` | `StateFreshnessBundle`, `TypeError`, `ValueError`, `isfinite` |

### 📄 scanner/state/invalidation.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_compute_structural` | `_g1_phase_to_none`, `_g2_insufficient_tier1` | `append` |
| `_compute_timing` | — | `append` |
| `_require_input_types` | — | `TypeError` |
| `_require_matching_identity` | — | `ValueError` |
| `compute_invalidation_and_cycle` | `_compute_structural`, `_compute_timing`, `_require_input_types`, `_require_matching_identity` | `InvalidationCycleBundle`, `ValueError`, `resolve_cycle_state` |

### 📄 scanner/state/machine.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_confirmed_ready_admitted` | `_meets_max`, `_meets_min` | — |
...
| `_validate_non_negative_int` | — | `ValueError` |
| `_validate_positive_price` | — | `ValueError`, `isfinite` |

### 📄 scanner/storage/repositories.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `apply_state_persistence_patch` | — | `execute` |
| `get_ohlcv_cache_meta` | — | `OhlcvCacheMetaRecord`, `execute`, `fetchone` |
| `insert_symbol_run_decision` | — | `execute`, `get`, `join` |
| `load_persisted_state_machine_context` | — | `PersistedStateMachineContext`, `execute`, `fetchone` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_parse_daily_bar_id` | — | `ValueError`, `date`, `strptime` |
| `_validate_partition_year_month` | — | `ValueError` |
| `_validate_relative_root` | — | `ValueError`, `replace`, `startswith`, `strip` |
| `_validate_run_id` | — | `ValueError`, `strip` |
| `_validate_symbol` | — | `ValueError`, `strip` |
| `_validate_timeframe` | — | `ValueError`, `join` |
| `build_ohlcv_history_partition_dir` | `_validate_partition_year_month`, `_validate_relative_root`, `_validate_symbol`, `_validate_timeframe` | `PurePosixPath`, `as_posix` |
| `build_run_manifest_path` | `build_run_snapshot_dir` | — |
| `build_run_snapshot_dir` | `_parse_daily_bar_id`, `_validate_relative_root`, `_validate_run_id` | `PurePosixPath`, `as_posix` |
| `is_month_open` | `_validate_partition_year_month` | `ValueError` |
| `month_mutability_policy` | `is_month_open` | `MonthMutabilityPolicy` |

### 📄 scanner/storage/sqlite.py

| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `connect_sqlite` | — | `Path`, `connect`, `execute`, `mkdir` |
| `init_db` | `connect_sqlite` | `apply_schema`, `close` |
...
| `_utc_iso` | — | `strftime` |
| `_utc_now` | — | `now` |
| `build_parser` | — | `ArgumentParser`, `add_argument`, `join` |
...
| Calling Function | Internal Calls | External Calls |
|------------------|----------------|----------------|
| `_base_symbol` | — | `endswith` |
| `classify_symbol` | `_base_symbol` | `UniverseClassification`, `endswith`, `search` |
...
| `scanner/runners/daily.py` | 16 | 92 | 108 | 🔴 High |
...
| `scanner/tools/export_evaluation_dataset.py` | 11 | 33 | 44 | 🔴 High |
...
| `scanner/evaluation/historical_replay/production_adapter.py` | 4 | 32 | 36 | 🔴 High |
...
| `scanner/decision/reasons.py` | 0 | 2 | 2 | 🔴 High |
| `scanner/state/cycle.py` | 1 | 1 | 2 | ⚠️ Medium |
...
- `docs/AI_CONTEXT_CURRENT.md` — AI routing + active/legacy context
- `docs/canonical/WORKFLOW_CODEX.md` — Codex implementation workflow
- `docs/GPT_SNAPSHOT.md` — current AI onboarding context
- `docs/archive/ai_context/` — archived AI context history (optional reference)
- `reports/runs/YYYY/MM/DD/<run_id>/` — per-run report artifacts
- `reports/daily/YYYY/MM/DD/report.json` — canonical daily report path
- `snapshots/runs/YYYY/MM/DD/<run_id>/run.manifest.json` — canonical run manifest path

---

_Generated by GitHub Actions • 2026-06-14 11:35 UTC_
