...
- `.env.example`
- `tools/ai_sparring/requirements.txt`
...
- **Official SDKs:** Use the official Python SDKs for both providers if available in `tools/ai_sparring/requirements.txt`. Do not build custom direct-HTTP clients unless the SDK blocks a required behavior.
- **OpenAI scope restriction in this ticket:** Use Chat Completions only for plain text request/response generation. Do not use built-in tools, file search, web search, computer use, or stored-response chaining in this ticket.
...
- **Provider roles:** A session has exactly two roles:
  - `drafter`
  - `reviewer`
- **Mode semantics:** `mode` changes only the resolved role-specific default system prompts in this ticket. It does **not** change:
  - context loading rules
  - artifact filenames
  - artifact schema
  - round structure
- **Default system prompts are required:** Implement one fixed built-in default system prompt per `(role, mode)` pair. Do not leave role behavior implicit.
- **No prompt-override flags in this ticket:** Do not add `--drafter-system-prompt` or `--reviewer-system-prompt` yet. Use fixed built-in defaults only.
- **Required default prompt behavior by mode:**
  - `ticket_review`
    - `drafter`: produce a concrete improved draft/spec/ticket based on the user prompt and visible context
    - `reviewer`: critically review the draft for ambiguity, missing defaults, missing edge cases, weak acceptance criteria, and anything that would force Codex to guess
  - `implementation_planning`
    - `drafter`: produce a concrete implementation plan with steps, boundaries, risks, and test considerations
    - `reviewer`: critically review the plan for sequencing gaps, hidden assumptions, missing rollback/safety constraints, and missing testability
  - `roadmap_review`
    - `drafter`: produce a structured roadmap analysis with priorities, dependencies, and recommended next steps
    - `reviewer`: critically review the roadmap analysis for prioritization mistakes, missing dependencies, hidden scope expansion, and vague outcomes
- **Persist resolved prompt identity:** Persist in `session.json` at least the resolved prompt identifiers for both roles, for example:
  - `resolved_prompts.drafter`
  - `resolved_prompts.reviewer`
  The ticket does not require persisting the full prompt text.
...
- `mode` must be one of:
  ...
  - `implementation_planning`
  - `roadmap_review`
- mode behavior in this ticket is defined **only** by the fixed built-in default system prompt pair for the selected mode
...
Implement fixed built-in default prompts for every `(role, mode)` pair.
...
Prompt text does not need to be identical to any prior draft, but it must faithfully implement the required behavior from the guardrails section:
- drafter = produce structured forward output for the selected mode
- reviewer = produce critical no-guesswork review for the selected mode

Persist resolved prompt identifiers in `session.json`.
...
- checkout repo
- setup Python
- install root requirements needed by the repo test environment
- install `tools/ai_sparring/requirements.txt`
- pass secrets via environment
- run the CLI
- upload the generated output artifacts

### Tool-local requirements
Create and use:

- `tools/ai_sparring/requirements.txt`

Add only the sparring-tool runtime dependencies there, including the provider SDKs needed by this ticket.

Do not add `openai` or `anthropic` to the repo-root `requirements.txt` in this ticket.
...
7) For each supported `mode`, the runtime resolves one deterministic built-in prompt identifier for:
   - `drafter`
   - `reviewer`
   and persists those identifiers in `session.json`.

8) For the same prompt, same providers, same models, and same context sources, changing `mode` changes the resolved role prompt identifiers but does not change:
   - context loading order
   - round structure
   - artifact filenames
   - session schema version

9) A completed 2-round session persists, for each round, exactly:
   - `draft`
   - `review`
   - `revision`
   - `delta_summary`

10) In round 2 and later, the drafter input includes both:
   - `review_r`
   - `revision_r`
   from the immediately preceding round.

11) If a transient provider error occurs and then succeeds on a later retry within the allowed retry budget, the session completes successfully and the recorded provider call shows `attempts_used > 1`.

12) If a fatal provider error occurs after preflight and before any successful protocol step, session artifacts are still written and `session.json` has `status=failed_runtime`.

13) If a fatal or exhausted-transient provider error occurs after at least one successful protocol step, session artifacts are still written and `session.json` has `status=failed_partial` with all earlier successful protocol data preserved.

14) `final_summary.md` is generated locally from structured session data and does not require an additional provider API call.

15) The workflow installs sparring-tool provider dependencies from `tools/ai_sparring/requirements.txt` without requiring `openai` or `anthropic` to be added to the repo-root `requirements.txt`.

16) The manual GitHub Actions workflow supports real-provider runs and uploads the produced artifacts.

17) Fake-provider execution from Ticket 01 continues to work after this ticket.

## Default-/Edgecase-Abdeckung (required for code tickets)

- **Config Defaults (Missing key -> Default):** ✅ (AC: #2, #3, #17 ; Test: `test_cli_runtime_defaults_keep_fake_providers`)
...
- **Nullability / no bool() coercion:** ✅ (AC: #1, #9 ; Test: `test_fake_provider_model_persists_as_null`)
- **Not-evaluated vs failed separated:** ✅ (AC: #12, #13 ; Test: `test_failed_runtime_vs_failed_partial_statuses`)
...
- **Deterministic sorting / tie-breakers:** ✅ (AC: #2, #3, #8 ; Test: `test_context_sources_have_stable_ordering`)
...
- Unit:
  ...
  - `test_draft_round_two_sees_prior_review_and_revision`
  - `test_modes_resolve_distinct_role_prompt_ids`
  - `test_failed_runtime_vs_failed_partial_statuses`
  - `test_tool_dependencies_are_declared_in_tool_local_requirements`

- Integration:
  - CLI run with mocked OpenAI and Anthropic SDK clients that completes 2 rounds and verifies the `session.json` contract
  - CLI run with two different modes and verifies that resolved prompt identifiers differ while context ordering and schema remain unchanged
...
- [x] Provider SDK dependencies remain isolated in `tools/ai_sparring/requirements.txt`
