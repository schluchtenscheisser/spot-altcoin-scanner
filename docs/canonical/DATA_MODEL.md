...
| `entry_location_candidate_segments.buy_now_candidates` | Daily report segment block | Report-side segment for rows whose `entry_action_hint` is `buy_now_candidate`. There is no diagnostics boolean named exactly `buy_now`. |
| `entry_location_candidate_segments.early_watch_candidates` | Daily report segment block | Report-side monitor segment for early fresh/acceptable rows. This is separate from the general report `watchlist` symbol list. |
| `entry_location_candidate_segments.avoid_chasing` | Daily report segment block | Report-side segment for rows whose `entry_action_hint` is `avoid_chasing`. There is no current field/value named exactly `avoid_chase`. |
