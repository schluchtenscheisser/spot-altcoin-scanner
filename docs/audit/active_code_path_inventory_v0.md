...
| `scanner/tools/backfill_snapshots.py` | direct CLI/tests | `scanner.pipeline.run_pipeline`, `scanner.pipeline.snapshot` | `active_analysis_script` | medium | Historical backfill utility that can execute legacy pipeline path. |
...
| `scanner/tools/backfill_snapshots.py` | Snapshot backfill | `active_analysis_script` | medium | snapshots dir from config | Can call legacy `scanner.pipeline.run_pipeline`. |
...
| `scanner/tools/backfill_snapshots.py` | Snapshot backfill | `active_analysis_script` | medium | Executable tool/tests; can call `scanner.pipeline.run_pipeline` | no | no | yes | no | snapshots | yes | candidate_for_future_legacy_isolation_review | Tool-only, pipeline-dependent. |
...
| `scanner/pipeline/__init__.py` | Pre-Independence pipeline orchestrator | `active_analysis_script` | medium | Self-labeled legacy; invoked by backfill tool/tests, not current daily/intraday | no | no | yes | no | legacy reports/snapshots | yes | clarify_authority_conflict | Should not be documented as active scanner runtime without follow-up. |
...
| `scanner/pipeline/scoring/*` | Legacy scoring implementations | `legacy_residue_test_only` | weak | Extensively tested; no active daily/intraday runner import | no | no | no | yes | none | yes | candidate_for_future_test_semantics_review | Includes `base_score`, multiplier, BTC-regime scoring. |
...
|---|---|---|---|---|---|
| `scanner/pipeline/` | Package has a top docstring: legacy/non-authoritative for Independence flows; not imported by daily/intraday runners; imported by tools/tests and indirectly active via execution liquidity dependency | `ambiguous` | medium | clarify_authority_conflict | Overall package is mixed: mostly legacy/test/tool, with one active utility dependency. |
...
| `scanner/pipeline/scoring/` | Contains `base_score`, multiplier-style scoring, BTC-regime scoring contexts; tests import extensively | `legacy_residue_test_only` | weak | candidate_for_future_test_semantics_review | No active runtime/evaluation workflow call found except through legacy pipeline/backfill possibilities. |
...
| `scanner/tools/backfill_snapshots.py` | Tool can call legacy `scanner.pipeline.run_pipeline` | `active_analysis_script` | medium | candidate_for_future_legacy_isolation_review | Not active runtime; script-only. |
...
| `base_score`, `multiplier` | Found in pipeline scoring/tests | `legacy_residue_test_only` | weak | candidate_for_future_test_semantics_review | Legacy scoring semantics. |
...
- `scanner.pipeline`, `pipeline.decision`, `pipeline.global_ranking`, `pipeline.scoring`, `compute_global_top20`, `base_score`, `multiplier`, `btc_regime`, and `GLOBAL_RANKING_TOP20` appear heavily in tests for pre-Independence/legacy pipeline behavior and some retained evaluation tooling.
...
| Tests for pipeline scoring/ranking | They may be intentionally retained compatibility/golden coverage, but audit cannot decide rewrite/removal | weak | candidate_for_future_test_semantics_review |
...
- `risk_level: medium`
- Evidence: Many tests import `scanner.pipeline.features`, `scanner.pipeline.scoring.*`, `scanner.pipeline.global_ranking`, `scanner.pipeline.output`, and `scanner.pipeline.backtest_runner`; active daily/intraday runners do not import these old scoring/output modules.
- Why it matters: Tests can keep old business semantics alive and confuse future changes unless test intent is explicitly labeled current, compatibility, or legacy.
- Recommended audit follow-up hint: `candidate_for_future_test_semantics_review`.
...
- `risk_level: high`
- Evidence: Legacy pipeline modules contain scoring/ranking/output/report semantics (`global_score`, `GLOBAL_RANKING_TOP20`, `base_score`, multiplier, report-side manifest behavior) that differ from current runner/report-builder paths.
- Why it matters: DOC-D current-state docs could accidentally document old scanner scoring/output semantics as current behavior.
